...
if needs_rebuild build/schwung-shim.so \
    ...
    # and for an LD_PRELOAD shim that means the failure lands at LOAD -- so
    # MoveOriginal does not start, and the device crash-loops with nothing in
    # dmesg. Measured: one `static` on a function called from another
    # translation unit cost exactly that. The shim resolves everything it uses
    # from itself or from libs listed here, so this is checkable at build time
    # and now is.
