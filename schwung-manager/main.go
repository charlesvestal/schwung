...
func main() {
	...
	mux.HandleFunc("GET /api/mcp/status", app.handleMoveMCPStatus)
	mux.HandleFunc("GET /api/mcp/capabilities", app.handleMoveMCPCapabilities)
	mux.HandleFunc("GET /api/mcp/sets", app.handleMoveMCPSets)
	mux.HandleFunc("POST /api/mcp/sets/select", app.handleMoveMCPSelectSet)
	mux.HandleFunc("POST /api/mcp/sets/duplicate", app.handleMoveMCPDuplicateSet)
	mux.HandleFunc("GET /api/mcp/current-set", app.handleMoveMCPCurrentSet)
	mux.HandleFunc("GET /api/mcp/current-set/settings", app.handleMoveMCPGetSongSettings)
	mux.HandleFunc("POST /api/mcp/current-set/settings", app.handleMoveMCPWriteSongSettings)
	mux.HandleFunc("POST /api/mcp/current-set/clips", app.handleMoveMCPWriteClip)
	mux.HandleFunc("GET /api/mcp/current-set/tracks/{track}/devices", app.handleMoveMCPGetTrackDevices)
	mux.HandleFunc("POST /api/mcp/current-set/tracks/{track}/devices", app.handleMoveMCPWriteTrackDevices)
	mux.HandleFunc("POST /api/mcp/current-set/tracks/{track}/devices/parameters", app.handleMoveMCPWriteDeviceParameters)
	mux.HandleFunc("GET /api/mcp/samples", app.handleMoveMCPSamples)
	mux.HandleFunc("POST /api/mcp/samples/index", app.handleMoveMCPWriteSampleIndex)
	...
	handler = middleware.CSRFProtectionWithConditionalExemptions(handler, []string{"/ws/"}, func(r *http.Request) bool {
		return strings.HasPrefix(r.URL.Path, "/api/mcp/") && app.hasValidMoveMCPToken(r)
	})
