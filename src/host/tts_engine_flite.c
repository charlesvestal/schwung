...
void flite_tts_set_speed(float speed) {
     ...
     * undefined symbol, so this built green and would have failed at the
     * CALL -- the first time anybody changed the screen reader's speed or
     * pitch. Line 405 already called the right one. */
    flite_save_config();
}
