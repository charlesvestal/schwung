...
static void shadow_inprocess_process_midi(void) {
    ...
    for (int i = 0; i < MIDI_BUFFER_SIZE; i += 4) {
        ...
        if (cin >= 0x08 && cin <= 0x0E && (status_usb & 0x80)) {
            ...
            /* Diagnostic: log note events on cable 0 and cable 2 from MIDI_OUT */
            if ((type == 0x90 || type == 0x80) && p2 >= 10 && (cable == 0 || cable == 2)) {
                static int mout_log = 0;
                if (mout_log < 200) {
                    char dbg[80];
                    snprintf(dbg, sizeof(dbg), "MIDI_OUT: cable=%d %s n=%d v=%d ch=%d",
                             cable,
                             (type == 0x90 && p3 > 0) ? "ON" : "OFF",
                             p2, p3, status_usb & 0x0F);
                    shadow_log(dbg);
                    mout_log++;
                }
            }
...
static void shim_post_transfer(void *ctx, uint8_t *shadow, const uint8_t *hw, int size)
{
    ...
    shadow_drain_midi_inject();
        ...
        /* === MOVE MIDI FX: Read chain output → inject into MIDI_IN cable-2 ===
         * MINIMAL VERSION: No cable-0 filtering. Testing if cable-0 + cable-2
         * in the same MIDI_IN frame causes SIGABRT (or if the old crash was
         * from the 4-byte stride bug, now fixed). */
        {
            ...
            /* Drain pending into MIDI_IN.
             * CRITICAL: MIDI_IN events are 8 bytes: [USB-MIDI(4)][timestamp(4)].
             * Move's ProcessEventsStepper asserts timestamps are monotonically increasing.
             * Find the max timestamp from existing events and use max+1 for injected events. */
            if (mfx_pending_count > 0) {
                uint8_t *mi = shadow + MIDI_IN_OFFSET;

                /* Find max timestamp from existing events */
                uint32_t max_ts = 0;
                for (int j = 0; j < MIDI_IN_MAX_BYTES; j += MIDI_IN_EVENT_SIZE) {
                    if ((mi[j] & 0xFF) == 0) continue;
                    uint32_t ts;
                    memcpy(&ts, &mi[j + 4], 4);
                ...
                while (src < mfx_pending_count && injected < 16) {
