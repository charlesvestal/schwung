...
/* --- RNBO Runner DB Import ---
 * Read the RNBO Runner sqlite DB and generate pack directories so its
...
/* Append a JSON-escaped string to a dynamic buffer.
...
/* Append raw string (no escaping) to dynamic buffer */
...
/* Export RNBO Runner sets from the sqlite DB as pack directories.
 * For each set, generates packs/<safe-name>/info.json and
 * packs/<safe-name>/sets/<safe-name>.json */
static void export_rnbo_runner_sets(const char *packs_path) {
    struct stat db_st;
    if (stat(RNBO_RUNNER_DB, &db_st) != 0) return;  /* DB doesn't exist */
    ...
    while (sqlite3_step(sets_stmt) == SQLITE_ROW) {
        ...
        /* Skip if info.json already exists and is newer than the DB */
        char info_path[1024];
        ...
        printf("shadow_ui: exporting RNBO Runner set '%s'\n", set_name);

        /* --- Build info.json --- */
        /* Query patcher instances for this set (deduplicate patchers by name+so_path) */
        ...
        /* Collect unique patchers and instances */
        #define MAX_INST 32
        struct {
            int instance_index;
            ...
            const char *config_json;  /* pointer into sqlite memory, valid until next step/finalize */
        } instances[MAX_INST];
        char *config_copies[MAX_INST];  /* heap copies of config JSON */
        int inst_count = 0;

        /* Track unique patchers for info.json */
        ...
        /* Check that at least one .so exists in cache */
        int any_so_exists = 0;
        ...
        if (!any_so_exists) {
            printf("shadow_ui: skipping set '%s' — no .so files found in cache\n", set_name);
            for (int i = 0; i < inst_count; i++) free(config_copies[i]);
            continue;
        }

        /* Build info.json string */
        ...
        for (int i = 0; i < inst_count; i++) {
            ...
            append_raw(&buf, &len, &cap, tmp);
            /* Config is already JSON — embed as-is */
...
/* JS binding: host_export_rnbo_runner_sets(packs_path) */
static JSValue js_host_export_rnbo_runner_sets(JSContext *ctx, JSValueConst this_val,
                                                int argc, JSValueConst *argv) {
    (void)this_val;
    if (argc < 1) return JS_UNDEFINED;
    const char *packs_path = JS_ToCString(ctx, argv[0]);
    if (!packs_path) return JS_UNDEFINED;
    export_rnbo_runner_sets(packs_path);
    JS_FreeCString(ctx, packs_path);
    return JS_UNDEFINED;
}
    ...
    for (int s = 0; subdirs[s] != NULL; s++) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (scan_packs[0]) {
                ...
                export_rnbo_runner_sets(packs_dir);
...
static void init_javascript(JSRuntime **prt, JSContext **pctx) {
    ...
    JS_SetPropertyStr(ctx, global_obj, "host_export_rnbo_runner_sets", JS_NewCFunction(ctx, js_host_export_rnbo_runner_sets, "host_export_rnbo_runner_sets", 1));
