#!/usr/bin/env bash
# Source pin: "Stay in Schwung" (Global Settings -> Display -> Track Tap).
#
...
echo "PASS: Stay in Schwung — live SHM byte at the C struct's offset, jump raised on the press outside the long-press gate, dismiss guarded, Shift+Track still exits, preserved across installs"
