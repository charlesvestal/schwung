...
/* THE ENTER ACK, as captured off the wire 2026-09-24: F0 00 21 5B 02 01 53 F7
 * -- NO 0x06, whatever the sheet says. The pre-release 06 53 form is not
 * accepted: that firmware cannot reach Move without a hand-patched image. */
eq("the captured ack is ACK_BODY", ACK_BODY, [0x00,0x21,0x5B,0x02,0x01,0x53]);
eq("the captured ack is an ack", isAck([0x00,0x21,0x5B,0x02,0x01,0x53]), true);
eq("the sheet 06 53 form is not", isAck([0x00,0x21,0x5B,0x02,0x01,0x06,0x53]), false);
/* Same header, same 0x53 -- payload length is the only thing separating an
 * ENTER ack from an OLED UPDATE ack, in both directions. */
eq("the ENTER ack is NOT an oled reply", parseOledUpdateReply(ACK_BODY), null);
