...
# The editor's own item count must be derived from the cap (Target N and
# Amount N are separate navigable items, alternating -- no separate Name
# row), not a restated literal that a cap raise would silently leave behind.
command grep -q 'return MACRO_MAX_TARGETS \* 2;' "$JS" \
