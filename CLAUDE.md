...
- **`access: "read"` is a STROKE, not a widget** — a dotted frame on the cell
  rect, added to whatever the value already draws. The input layer honoured
  readOnly for a long time while the DRAW layer did not, so a readout was
  pixel-identical to a control.
